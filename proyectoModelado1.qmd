...
function solve_tsp(coordinates)
    ...
    total_distance =sum(dist[i, j] * solution[i, j] for i in 1:n for j in i+1:n)
...
```{julia}
using JuMP, HiGHS, LinearAlgebra, Gurobi
...
function solve_tsp(coordinates)
    ...
    @variable(model, 2<= u[2:n] <= n, Int)

    @objective(model, Min, sum(dist[i, j] * x[i, j] for i in 1:n for j = 1:n  if j != i))
    ...
    solution = value.(x)
    total_distance = sum(dist[i, j] * solution[i, j] for i in 1:n for j = i+1:n)
